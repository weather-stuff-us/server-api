...
const urls = require('./weather-api/lib/urls')
...
async function forLatLon (latArg, lonArg) {
  ...
  const result = buildResult(lat, lon, metaData.properties, forecastSummary.properties)

  LocationInfoCache.set(key, result)
  return result
}

function buildResult (lat, lon, metaData, forecastSummary) {
  const result = {}
