...
addConverter('null', null, value => value)
addConverter('degC', 'f', value => utils.round((value * 9 / 5) + 32))
addConverter('degree_(angle)', 'compass', degreesToCompass)
addConverter('m', 'ft', value => utils.round(value * 3.28084, 1))
addConverter('m', 'mi', value => utils.round(value / 1609.34), 1)
addConverter('m_s-1', 'mph', value => utils.round(value * 2.23694))
addConverter('mm', 'in', value => utils.round(value / 25.4, 2))
...
function convertIt (value, fromCode, toCode) {
  ...
  console.log(`${value} ${fromUnit.code} == ${converted} ${toUnit.code}`)
}
